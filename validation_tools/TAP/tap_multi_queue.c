...
int tap_alloc_mq(char *dev, int queues, int *fds)
{
    struct ifreq ifr;
    int fd, err, i;
...
{   
    if(argc != 4) {
        fprintf(stderr, "Usage: ./tap_multi_queue.o <num_targets> <wait_time_nanos> <run_time_ns>\n");
        return 1;
    }
    ...
    int init = tap_alloc_mq("tap0", num_targets, fds);
    if (init) {
        printf("Error while init tap0: %d\n", init);
        return init;
    }
